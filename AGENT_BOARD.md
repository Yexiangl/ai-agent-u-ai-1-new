...
| TASK-051 | 待实测 | P2 | OpenClaw 原生化优化（第2档 C+D + 第3档 E） | 续 TASK-050，用户要求做第二+三档。**实测调研**：web_search 返回 details.results[]={title,url,snippet,siteName}，title/snippet 裹 `<<<EXTERNAL_UNTRUSTED_CONTENT id=..>>>`+Source:+--- 需剥离；普通聊天默认不主动联网；sessions_list 字段齐全；WS 端点用项目 openclawGateway.ts 精确帧（v3 设备签名 payload 顺序=v3\|deviceId\|clientId\|clientMode\|role\|scopes\|signedAt\|token\|nonce\|platform\|，client.id 须='gateway-client' 枚举值，minProtocol/maxProtocol=4）实测 backend 模式 loopback CONNECT ok=true、protocol4、server.version 2026.5.27、features.methods 100+。CSP=null 允许 WebView 直连 ws://。**关键发现**(protocol.md:314-323)：WS 广播 agent/chat/tool-result 帧对 operator.read 客户端开放→HTTP 聊天触发的 run 也会广播。实测聊天期间收到 `agent` 事件：stream:lifecycle(start/end)、stream:assistant(text/delta)、stream:item(kind:tool,phase:start/update/end,status:running/completed/failed,name:web_search/web_fetch/exec,title,itemId)、stream:command_output。**C 聊天联网开关**：Rust openclaw_web_search（invoke_gateway_tool 共享助手 + strip_untrusted_wrapper 剥包装，实测剥离后干净）+ openclawHttpClient openClawWebSearch/类型；前端输入框「联网」开关(Globe,仅 OpenClaw)，send() 开启时先搜前5条注入 grounding 上下文+要求标引用号，UiChatMessage 加 sources 字段，气泡底「联网来源」可点列表(open_url)，新增 searching 阶段+占位「正在联网搜索」。**D 会话可视化**：Rust openclaw_sessions_list（含 totalTokensAcrossSessions 汇总）+前端类型；EnginesPage 加「会话活动」面板(会话数+累计 token + 每条状态点/agent/模型/token)。**E WS 工具进度**：openclawBackend 加 connectToolEvents(onTool)/disconnectToolEvents + OpenClawToolItem/OpenClawGatewayConnState 类型（复用现成 openclawGateway.ts 客户端，新建 OpenClawGatewayClient(undefined,token)→connect()→onEvent 过滤 agent+stream:item+kind:tool）；App.tsx 加 wsToolState/wsToolUnsubRef/wsToolItemsRef，openclawConnected 时 effect 连 WS 订阅工具事件→formatToolItem(中文工具名+状态,跳过 update 相)→按 activeRequestRef 附到当前 assistant 消息 toolEvents（复用现成 ToolsBlock 渲染），send() 清 dedup ref，header 加「工具流」指示(hover 显协议)。聊天流不动(仍 HTTP SSE)，WS 纯旁路只读、best-effort 不阻塞。cargo check(仅 3 个无关 snake_case 预存警告)+tsc+build+redaction 21/21 全过；实测 WS 握手 ok=true/protocol4 + 强制模型调 web_search/web_fetch/exec 收到完整 tool item 事件序列。待用户 tauri dev 实测联网开关/来源展示/会话面板/工具流进度。 |
 ...
 | TASK-068 | 已完成待实测 | P0 | v0.1.3 实测二轮反馈:更新黑窗仍在(版本时间差) + 装完仍"未找到配置"(PATH未刷新) | 用户 v0.1.2→v0.1.3 更新仍弹黑窗、一键安装后仍"未找到本地配置文件"。**根因考察**：**问题1(更新黑窗)**——非新bug,是**版本时间差**:执行 swap 的 apply_update 代码属于**当前运行的版本**(v0.1.2,旧buggy代码),修复在 v0.1.3,故本次仍弹黑窗;**v0.1.3→v0.1.4 起才生效**(git log -S CREATE_NO_WINDOW 确认修复仅在 419bd67/v0.1.3)。**问题2(未找到配置)**——拆成两个独立真问题:**A(核心,PATH未刷新)**:openclaw 装在 `%APPDATA%\npm\openclaw.cmd`,官方脚本把该路径加进**用户PATH**,但 App 是安装前启动的、进程PATH是旧的→`openclaw --version` 找不到→误判未装(openclaw 官方FAQ 明确记载此坑「openclaw is not recognized 需开新终端」,Context7 查证);**B(--no-onboard 不生成配置)**:`~/.openclaw/openclaw.json` 由 onboarding 向导创建,加了 --no-onboard 跳过向导→装完无配置文件属预期,但UI显示得像报错。**修复(与用户确认两个全做)**：**①不依赖PATH检测+调用**(main.rs)新增 `resolve_openclaw_bin()`(先试裸PATH探测,失败则查已知位置:Win `%APPDATA%\npm\openclaw.cmd`+`%USERPROFILE%\.local\bin\openclaw.cmd`;mac `~/.local/bin`、`/opt/homebrew/bin`、`/usr/local/bin` 等,OnceLock缓存)+`openclaw_known_paths()`+`openclaw_command()`(解析全路径构造Command,Win下`.cmd`/`.bat`走`cmd /c`);**perl 全局替换** 17处 `std::process::Command::new("openclaw")`→`openclaw_command()`(resolver内裸probe不动);**②check_openclaw_installed 重写**不依赖PATH:on_path(裸探测)||known文件存在→installed,额外返回 `onPath` 区分「没装」vs「装了但需重启」。**③前端引导重启**(App.tsx/openclawInstaller.ts):OpenClawInstallStatus 加 onPath;OpenClawInstallCard 装完 done 后 recheck——onPath 真→隐藏走配置流;installed 但 !onPath→新增 `needsRestart` 态显示绿色「本地服务安装完成,请完全关闭并重开程序」卡。**改动文件**:main.rs(+resolver/known_paths/command 三helper+17处替换+check重写)、App.tsx(needsRestart 态+View)、openclawInstaller.ts(onPath)。**验证**:tsc 0 错;build 通过(498KB);cargo check 无错;Playwright 烟测安装卡正常渲染不崩。⚠️**待实测(随 v0.1.4)**:①**更新黑窗**:v0.1.3→v0.1.4 这次应无黑窗+.bak自动清理(首次能真正验证 TASK-067 的 swap 修复)②**一键安装**:装完 App 能否经 resolve_openclaw_bin 直接探测到(免重启);若仍需重启则显示 needsRestart 绿卡引导③重启后能否正常走「填密钥启用」生成配置④全路径调用 openclaw 各命令(gateway/skills/channels等17处)在真机是否正常。 |
 | TASK-067 | 已完成待实测 | P0 | 真机实测反馈修复:更新swap黑窗 + 安装onboarding卡死 + 日志丢失 | 用户在 Windows 虚拟机用 v0.1.1 实测 TASK-065/066,报三个问题,全部修复。**问题1(P0,更新 .bat swap)**：点立即安装弹出可见 cmd 黑窗、报「找不到批处理文件」、`Z:\>` 提示符、U盘残留 `exe.bak`。根因:①`cmd /c start "" bat` 开可见窗口②`del "%~f0"` 经典自删报错③没清理 .bak。**修复**(update.rs apply_update Windows 分支重写)：①改 `Command::new("cmd").args(["/c", bat]).creation_flags(CREATE_NO_WINDOW|DETACHED_PROCESS)` 隐藏+脱离父进程②`timeout`(需 stdin,无窗口会失败)换成 `ping 127.0.0.1 -n 4 >nul` 延时③自删改用规范写法 `(goto) 2>nul & del "%~f0"`④swap 后加 `ping`延时 + `del /q "{bak}"` 清理备份⑤所有 move 加 `2>&1` 抑制错误。注:用户报「程序版本已是最新」=swap 其实成功了,只是过程脏。**问题2(P0,一键安装卡死)**：用户指出应使用官方 `--no-onboard`/`-NoOnboard`。根因:官方安装脚本末尾跑**交互式 onboarding 向导**,headless(无TTY)运行会挂起。**修复**(main.rs build_openclaw_install_command)：Win 改 `& ([scriptblock]::Create((iwr -useb https://openclaw.ai/install.ps1))) -NoOnboard` + `creation_flags(CREATE_NO_WINDOW)`；mac 改 `curl -fsSL https://openclaw.ai/install.sh | bash -s -- --no-onboard`。**问题3(P1,日志丢失)**:切换页面 EnginesPage 卸载→install 卡的 useState 日志清空,需重新点。**修复**:openclawInstaller.ts 重写为**模块级单例 store**(state{phase,logs}+subscribers+ensureListeners 全局只 listen 一次),导出 subscribeInstall/getInstallState/startInstall;App.tsx 的 OpenClawInstallCard 改为 subscribe store(detection 本地 checking/missing/installed,install phase/logs 来自 store),切页面再回来日志/状态保留。另:安装中加蓝色提示「请耐心等待几分钟+若弹防火墙选允许」(用户报的 Node.js 防火墙弹窗是 Windows 正常行为非bug)。**改动文件**:update.rs、main.rs、App.tsx、openclawInstaller.ts(重写)。**验证**:tsc 0 错;build 通过(497KB);cargo check 无错(Windows-only 的 creation_flags/CommandExt 在 mac 经 cfg 跳过,CI 真机编译);**Playwright 烟测**——安装卡渲染、点按钮浏览器态失败兜底正常、**切到首页再回 AI助手页失败状态+日志持久保留**(store 生效)。⚠️**待实测(必须真机,随 v0.1.3)**:①更新 swap 黑窗是否消失、.bak 是否自动清理、重启是否正常②`-NoOnboard` 后安装是否不再卡死、能否跑完转 done③装完 PATH 未刷新可能仍需重启 App(官方已知问题)。 |
 | TASK-066 | 已完成待实测 | P1 | openclaw 一键安装功能(调官方安装脚本) | 用户需求：小白用户不会自己装 openclaw 环境，要在 App 里一键装。**考察结论**(读了官方 install.ps1 全文 1000+行 + install.sh 关键段)：openclaw 是**重量级 Node 应用**，官方脚本会①检测/装 Node 24(winget→choco→scoop→便携Node 下载到 `%LOCALAPPDATA%\OpenClaw\deps`)②检测/装 Git(MinGit)③`npm install -g openclaw`(几百MB)④配 PATH+跑 doctor+装 gateway 系统服务。**关键架构决策**(与用户确认)：openclaw **无法真正便携**(gateway 是 Windows 计划任务/mac LaunchAgent 绑电脑、npm 含原生编译模块换机崩、要改一堆环境变量)，最终定**「App+用户数据 便携(U盘)，openclaw 引擎装电脑(每台新机首次装一次)」**架构。**实现方式**(与用户确认)：不自研安装逻辑(官方脚本 1000 行维护成本极高)，**调用官方安装脚本**。**后端**(main.rs 加 2 命令并注册)：①`check_openclaw_installed`——`openclaw --version` 探测，返回 `{installed,version}`，用已有 `hide_command_window`(Win 隐藏控制台)②`install_openclaw`——`build_openclaw_install_command()` 按平台构造，spawn 后两个线程分别 BufReader 逐行 drain stdout/stderr→`emit("openclaw-install-log",{line})`(strip_ansi)，进程结束 emit `openclaw-install-done`{success}；模式抄 `start_wechat_login` 的流式输出。**前端**：①新增 `src/lib/openclawInstaller.ts`②App.tsx 加 `OpenClawInstallCard`+`OpenClawInstallCardView`(「安装本地服务」卡)，放在 EnginesPage「启用 AI 助手」卡之前。**注:命令构造与日志持久化在 TASK-067 被修正(改 --no-onboard + 模块级 store)。**安全边界**：执行远程官方脚本(openclaw 官方推荐标准装法)；命令参数硬编码无注入面；日志 strip_ansi。⚠️**待实测**:见 TASK-067。 |
