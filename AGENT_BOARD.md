...
| TASK-032A | ✅ 待验收 | P0 | 用量概览真实性审计与重构方案 | 审计完成：docs/usage-overview-audit.md。主要发现：(1) Token 全 0 因 Backend 丢弃 usage；(2) openclaw/default 为路由 ID；(3) OpenClaw 确实返回 usage。8 个推荐方案，7 个子任务。 |
...
- TASK-032A 已执行（2026-05-28）：用量概览真实性审计完成。输出 `docs/usage-overview-audit.md`，共 8 章节。关键发现：(1) Token 全 0 根因：`OpenClawBackend.startChat()` 丢弃了 Rust 层返回的 `usage` 字段；(2) `openclaw/default` 是 OpenClaw 内部路由 ID，不是用户模型名，在 12 处代码被硬编码为 `modelName`；(3) OpenClaw API 确实返回 usage 字段，Rust 层和 TypeScript 类型均完整，仅 Backend 适配层丢弃；(4) 会话数和消息数来自本地文件，数据真实。推荐方案：P0 先做 UI 文案修正（避免误导），P1 修复 usage 传递和保存，P2 模型名去内部化，P3 评估服务端统计。
