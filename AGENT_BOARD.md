...
| TASK-033A | ✅ 待验收 | P0 | 助手记忆数据源审计 | 审计完成：`docs/assistant-memory-source-audit.md`。确认当前读取 Hermes `~/.hermes/`，OpenClaw `~/.openclaw/workspace/` 含 6 个 .md 记忆文件未接入。建议新增 `read_openclaw_workspace_memory` Rust 命令。 |
| TASK-033B | 待规划 | P0 | OpenClaw 记忆只读命令 | Rust `read_openclaw_workspace_memory` + 前端接入，读取 `~/.openclaw/workspace/*.md`。 |
