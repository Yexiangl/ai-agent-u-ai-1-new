...
### 5.2 TASK-032C：用量概览 UI 修正 ✅ 已完成（2026-05-28）

**实现**：

| 改动 | 说明 |
|---|---|
| 标题 "使用概况" → **"本地用量概览"** | 明确数据来源 |
| 说明文案重写 | 强调 Token 来自 usage 字段，未返回时显示暂未提供 |
| 无 usage 时显示 **"暂未提供"** | 不自欺欺人显示 0 |
| 有 usage 时显示真实统计 + **"真实统计"** 提示 | 基于 N 条回复的 usage 字段 |
| 模型名 `openclaw/default` → **"默认模型"** | 去内部化 |
| 最近会话 Token 列按 session 内 usage 判定 | 不显示毫无意义的 `0 tokens` |
| 旧会话兼容 | 无 usage 的消息不计入、不报错 |

**不变量**：不做估算、不接后台、不暴露敏感信息。
